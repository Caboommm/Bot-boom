...
intents.members = True # <--- OBRIGATÓRIO PRO AUTO-ROLE FUNCIONAR
bot = commands.Bot(command_prefix='!', intents=intents)

# ==============================================================================
# ⚠️ ÁREA DE CONFIGURAÇÃO (PREENCHA AQUI) ⚠️

# 1. COLOQUE O TOKEN DO MERCADO PAGO AQUI
...
# 4. AUTO-ROLE (CARGO AUTOMÁTICO AO ENTRAR)
# Coloque o ID do cargo aqui embaixo (número inteiro, sem aspas)
ID_AUTOROLE = 1465012346794676249
...
# --- SISTEMA DE AUTO-ROLE (NOVO) ---
@bot.event
async def on_member_join(member):
    guild = member.guild
    # Tenta pegar o cargo pelo ID configurado lá em cima
    role = guild.get_role(ID_AUTOROLE)

    if role:
        try:
            await member.add_roles(role)
            print(f"✅ Cargo {role.name} dado para {member.name} com sucesso!")
        except discord.Forbidden:
            print(f"❌ ERRO: O bot não tem permissão para dar o cargo {role.name}. O cargo do bot deve estar ACIMA do cargo do membro.")
        except Exception as e:
            print(f"❌ Erro ao dar cargo: {e}")
    else:
        print(f"❌ ERRO: Não encontrei o cargo com ID {ID_AUTOROLE}. Verifique a configuração.")
